...
import internshipsPageInitialPaths4d402Resource from '../../resources/internships-page-initial-paths-4d402'
import internshipsPageInitialPropsE8ebbResource from '../../resources/internships-page-initial-props-e8ebb'
...
export async function getStaticPaths() {
  try {
    const response = await internshipsPageInitialPaths4d402Resource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await internshipsPageInitialPropsE8ebbResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
