...
import slidersPageInitialPaths61631Resource from '../../resources/sliders-page-initial-paths-61631'
import slidersPageInitialProps495dbResource from '../../resources/sliders-page-initial-props-495db'
...
export async function getStaticPaths() {
  try {
    const response = await slidersPageInitialPaths61631Resource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await slidersPageInitialProps495dbResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
