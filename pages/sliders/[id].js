...
import slidersPageInitialPaths55017Resource from '../../resources/sliders-page-initial-paths-55017'
import slidersPageInitialProps4b0ecResource from '../../resources/sliders-page-initial-props-4b0ec'
...
export async function getStaticPaths() {
  try {
    const response = await slidersPageInitialPaths55017Resource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await slidersPageInitialProps4b0ecResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
